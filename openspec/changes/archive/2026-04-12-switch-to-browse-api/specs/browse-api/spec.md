# Browse API Integration Specification

## ADDED Requirements

- **OAuth Token Management**: OAuth 2.0 Application Access Token from client_credentials flow with global token cache
- **Browse API JSON Client**: New JSON-based Browse API integration with offset-based pagination
- **Resilience**: Exponential backoff retry logic for 429 and 5xx errors; fail-fast on auth errors
- **Token Lifecycle**: Automatic token expiry refresh with 60s safety margin
