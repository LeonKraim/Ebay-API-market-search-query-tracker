...
- Unit tests for all services (ebay_finding, ebay_browse, sold_scraper, dedup, poll_runner)
- Integration tests for all routers
- Edge case tests: API errors (429, 503), DB constraint violations, HTML variants, scheduler overlap
- Mutation testing: inject defects, verify test fails, restore code

### Frontend Tests (Vitest + React Testing Library)
- Unit tests for all components
- Unit tests for all hooks
- Unit tests for lib functions (logger, format)
- E2E tests with Playwright: dashboard, add query, view results, filter, sort, scheduler, settings
- Mutation testing for all behaviors

## Performance Requirements

- Page load time: < 2 seconds (Vite cold + hot)
- API response time: < 500ms (p95)
- eBay API polling: max 3 concurrent requests, 1.5s delay between pages
- Database query: < 100ms for pagination
- NProgress: visible within 50ms of user action

## Security Requirements

- HTTPS in production (user's responsibility)
- CSRF token on state-changing POST/PATCH/DELETE (optional, can use CORS)
- No PII logging (credentials, API keys, user tokens redacted)
- Database credentials in `.env.example` format, never in code
- Bearer token compared using `secrets.compare_digest()`
...
- Log rotation (Loguru 50MB, 30 days)
...
- Malformed responses: log error, mark snapshot as failed
- Timeout: max 15 seconds per request

### Database Errors
- Unique violation on duplicate item: log and skip
- Foreign key violation: log error, mark snapshot as failed
- Connection pool exhausted: queue requests

### Scraper Errors
- HTML structure changed: log warning, attempt best-effort parsing
- Zero results on visible page: log warning, continue

### Scheduler Errors
- Concurrent poll overlap: skip and log
- Job crashes: log error, reschedule for next interval

## Logging Specification

Every function logs:
- Entry: `[MODULE] function starting with params`
- Exit: `[MODULE] function complete (duration: Xs)`
- Errors: `[MODULE] ERROR: message with context`

Every API call logs:
- Request: `[API] GET /path (params)`
- Response: `[API] 200 OK (duration: Xms, size: Xkb)`
- Error: `[API] 500 ERROR (message)`
...
- /health endpoint (implicit via FastAPI /docs)
- /stats/items-evaluated for UI counter
- Scheduler status shows next run times

## Backwards Compatibility

- API versioning: /api/v1/
- Schema migrations support data preservation
- Graceful deprecation of endpoints with version warnings
